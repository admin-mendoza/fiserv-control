...
def normalizar_comercio(valor):
    """Elimina ceros a la izquierda del número de comercio."""
    return str(valor).strip().lstrip('0') or '0'

def detectar_tarjeta(contenido):
    c = contenido.upper()
    if "DEBITO" in c and "VISA" in c:
        return "Visa Débito"
    elif "CREDITO" in c and "VISA" in c:
        return "Visa Crédito"
    elif ("DEBITO" in c or "DEBIT" in c) and "MASTER" in c:
        return "Mastercard Débito"
    elif ("CREDITO" in c or "CREDIT" in c) and "MASTER" in c:
...
def extraer_cupones_pdf(pdf_file):
    ...
    # Buscar número de comercio — acepta formatos con o sin ceros adelante
    comercio_match = re.search(r'N[°º\.]\s*Comercio[:\s]+0*(\d+)', contenido)
    if not comercio_match:
        comercio_match = re.search(r'Comercio[:\s]+0*(\d+)', contenido)
    comercio_raw = comercio_match.group(1).strip() if comercio_match else "0"
    comercio = normalizar_comercio(comercio_raw)

    tarjeta = detectar_tarjeta(contenido)
...
def generar_excel_plantilla():
    df = pd.DataFrame({
        "comercio": ["29992214", "29992250", "29992255"],
        "lote":     [149,        160,        138],
        "cupon":    ["0699",     "1111",     "0360"],
        "importe":  [9477.42,    243914.57,  12348.98],
    })
    buf = io.BytesIO()
...
with tab2:
    ...
    with st.expander("📥 ¿Cómo armar el Excel?", expanded=True):
        st.markdown("""
El Excel debe tener estas **4 columnas** (el importe es opcional pero recomendado para detectar diferencias):
...
- **importe** → el monto en $ impreso en el ticket

El número de comercio identifica la sucursal automáticamente.
El importe se puede escribir con punto o coma decimal (ej: 9477.42 o 9477,42).
        """)
        st.download_button(
    ...
    if excel_file:
        try:
            ...
            if faltantes:
                ...
                # Normalizar — misma lógica que los PDFs
                df_excel["comercio"] = df_excel["comercio"].apply(normalizar_comercio)
                df_excel["lote"]     = df_excel["lote"].str.strip().apply(lambda x: str(int(float(x))) if x else '0')
                ...
                cols_show = ["sucursal","lote","cupon","importe"] if "importe" in df_excel.columns else ["sucursal","lote","cupon"]
                st.dataframe(df_excel[cols_show], use_container_width=True, hide_index=True)
...
with tab3:
    ...
    for _, row in df_fis.iterrows():
        ...
        if clave in liq_dict:
            ...
            imp_liq = l["importe"]
            if imp_fis is not None and float(imp_fis) != 0:
